...
export function TypingPractice({ word, meaning, example, exampleTranslation, onSuccess, index, total }: TypingPracticeProps) {
 ...
 <div className="flex items-center gap-2 bg-[var(--color-foreground)] text-[var(--color-background)] px-4 py-1 border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow">
 <Sparkles className="w-4 h-4 text-amber-400" />
 <span className="text-[10px] font-black tracking-widest uppercase">단어 완성하기</span>
 </div>
 
 {example ? (
 <div className="space-y-2 w-full px-2">
 <div className={cn(
 "flex flex-wrap justify-center items-center gap-x-1 gap-y-1 font-black bg-[var(--color-surface)]/50 p-3 rounded-2xl border-2 border-dashed border-[var(--color-border)]/10 italic",
 ...
 <span key={i} className="text-[var(--color-foreground)]/80">{part}</span>
 )
 ))}
 </div>
 <p className="text-xs font-black text-[var(--color-primary)] italic leading-tight">
 &quot;{exampleTranslation}&quot;
 </p>
 </div>
 ) : (
 <h2 className="text-2xl sm:text-4xl font-black text-[var(--color-primary)] drop-shadow-sm">
 {formatWord(meaning)}
 </h2>
 )}
 ...
 className={cn(
 "flex items-center justify-center gap-1.5 p-2 border border-[var(--color-border)] transition-all",
 !isDirectMode 
 ? "bg-[var(--color-foreground)] text-[var(--color-background)]" 
 : "bg-[var(--color-surface)] text-[var(--color-foreground)]"
 )}
 >
 <Sparkles className={cn("w-3 h-3", !isDirectMode ? "fill-amber-400" : "text-[var(--color-foreground)]")} />
 <span className="text-[10px] font-black uppercase">버블</span>
 </button>
 <button 
 ...
 <span className="text-[10px] font-black uppercase">키보드</span>
 </button>
 </div>
 </div>
 </div>
 ...
 {/* Slots Area */}
 <div className={cn(
 "w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-4 sm:p-8 shadow-sm hover:shadow-md transition-shadow min-h-[100px] flex flex-wrap justify-center items-center gap-x-2 gap-y-4 transition-all",
 shake && "animate-shake border-error",
 status === 'success' && "border-success bg-[var(--color-success)]/5"
 )}>
 {isDirectMode ? (
 ...
 className={cn(
 "w-full bg-[var(--color-surface)] border-4 border-dashed border-[var(--color-border)]/20 p-6 text-center font-black italic outline-none rounded-3xl transition-all focus:border-primary/30",
 ...
 autoComplete="off"
 />
 <div className="absolute bottom-2 left-4 right-4 h-2 bg-[var(--color-muted)] rounded-full overflow-hidden">
 <motion.div 
 ...
 className={cn(
 "border-b-4 border-[var(--color-border)] flex items-center justify-center font-black transition-all italic",
 ...
 tile ? "bg-[var(--color-surface)] border-2 shadow-sm hover:shadow-md transition-shadow scale-105" : "border-[var(--color-border)]/10"
 )}
 >
 {tile?.char}
 </motion.button>
 ...
 onClick={() => speak(word)} 
 className={cn(
 "h-12 w-12 rounded-full border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow bg-[var(--color-surface)]",
 isPlaying ? "text-[var(--color-primary)] border-primary" : "text-[var(--color-foreground)]"
 )}
 >
 ...
 className="h-12 w-12 rounded-full border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow bg-[var(--color-surface)] text-amber-500 disabled:opacity-50"
 >
 ...
 variant="ghost" 
 onClick={handleClear} 
 className="h-12 w-12 rounded-full border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow bg-[var(--color-surface)] text-[var(--color-foreground)]"
 >
 ...
 className={cn(
 "bg-[var(--color-surface)] border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow flex items-center justify-center font-black italic hover:bg-[var(--color-muted)] transition-colors cursor-pointer active:translate-y-0.5 active:shadow-none",
 ...
 <div className="h-6">
 {status === 'error' && <p className="text-[var(--color-error)] font-black text-[10px] animate-bounce">다시 시도해보세요!</p>}
 </div>
 </div>
 );
}
