...
export function SentenceCompletion({ sentence, translation, targetWord, onSuccess, index, total }: SentenceCompletionProps) {
 ...
 <div className="inline-flex items-center gap-2 bg-secondary text-white px-3 py-1 border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow">
 <Sparkles className="w-4 h-4" />
 <span className="text-[10px] font-black tracking-widest uppercase">문장 완성하기</span>
 </div>
 <p className="text-sm font-black text-[var(--color-primary)] italic leading-tight">&quot;{translation}&quot;</p>
 </div>

 <div className={cn(
 "w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-4 sm:p-8 shadow-sm hover:shadow-md transition-shadow transition-all relative overflow-hidden",
 shake && "animate-shake border-error",
 status === 'success' && "border-success"
 )}>
 <div className={cn(
 "flex flex-wrap justify-center items-center gap-x-1 gap-y-2 font-black italic",
 ...
 key={i}
 className={cn(
 "min-w-[60px] sm:min-w-[100px] h-10 sm:h-12 border-b-4 border-[var(--color-border)] flex items-center justify-center transition-all px-2",
 selectedWord ? "text-[var(--color-primary)]" : "text-[var(--color-foreground)]/10"
 )}
 >
 ...
 className={cn(
 "flex items-center gap-2 border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm hover:shadow-md transition-shadow active:translate-y-0.5 text-[10px] transition-all px-4 py-2",
 isPlaying ? "text-[var(--color-primary)] border-primary shadow-none translate-y-0.5" : "text-[var(--color-foreground)]"
 )}
 >
 <Volume2 className={cn("w-4 h-4", isPlaying && "text-[var(--color-primary)] fill-primary/20")} /> 발음 듣기
 </Button>
 </div>
 </div>
 ...
 className={cn(
 "px-5 py-2.5 bg-[var(--color-surface)] border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow text-base sm:text-xl font-black italic hover:bg-[var(--color-muted)] transition-all text-[var(--color-foreground)]",
 selectedWord === opt && status === 'success' && "bg-[var(--color-success)] text-white border-success shadow-none translate-y-1",
 selectedWord === opt && status === 'error' && "bg-[var(--color-error)] text-white border-error shadow-none translate-y-1"
 )}
 >
 {formatWord(opt)}
 </motion.button>
