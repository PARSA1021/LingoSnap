...
export function ListeningQuizStep({
 ...
 <div className="inline-flex items-center gap-2 px-2 py-0.5 bg-[var(--color-info)] text-white border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow">
 <Volume2 className="w-4 h-4" />
 <span className="text-[10px] font-black tracking-widest ">리스닝 미션</span>
 </div>
 <h2 className="text-xl sm:text-3xl font-black text-[var(--color-foreground)] ">들리는 단어를 선택하세요</h2>
 </div>
 ...
 disabled={isLoading}
 className={cn(
 "w-24 h-24 sm:w-32 sm:h-32 rounded-3xl border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow flex items-center justify-center active:translate-y-1 active:shadow-none transition-all",
 isPlaying ? "bg-secondary text-white" : "bg-[var(--color-primary)] text-white",
 ...
 disabled={done}
 className={cn(
 "h-20 sm:h-24 px-3 rounded-2xl border-4 font-black text-xl transition-all italic",
 !done
 ? 'border-[var(--color-border)] bg-[var(--color-background)] shadow-sm hover:shadow-md transition-shadow active:translate-y-1 active:shadow-none'
 : isCorrect
 ? 'border-success bg-[var(--color-success)] text-white shadow-none'
 : isSelected
 ? 'border-error bg-[var(--color-error)] text-white shadow-none'
 : 'border-[var(--color-border)] bg-[var(--color-background)] opacity-30 grayscale shadow-none'
 )}
 >
