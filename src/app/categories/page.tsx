...
const categories = [
  { id: 'all', name: 'TOTAL RANDOM', icon: GraduationCap, desc: '모든 분야의 단어를 골고루 학습합니다.', color: 'bg-primary' },
...
export default function CategoriesPage() {
  return (
    <div className="min-h-screen w-full flex flex-col bg-background pt-24 md:pt-36 pb-32 px-6 md:px-12 overflow-x-hidden">
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-16 md:gap-24">
        ...
        <header className="flex flex-col gap-6 max-w-3xl">
          <motion.div
            ...
            className="inline-flex w-fit bg-black text-white text-[10px] md:text-xs font-black px-4 py-1.5 border-4 border-black uppercase tracking-widest shadow-[4px_4px_0_rgba(0,0,0,0.1)]"
          >
            Choose Your Focus
          </motion.div>
          <motion.h1 
            ...
            className="text-5xl md:text-8xl font-black text-foreground uppercase leading-[0.9]"
          >
              ...
              >
                <Link href={`/learn?category=${cat.id}`} className="block group h-full">
                  <div className={cn(
                    ...
                    cat.id === 'all' ? "bg-primary" : "bg-white dark:bg-black"
                  )}>
                    <div className="space-y-6">
                      <div className={cn(
                        "w-16 h-16 border-4 border-black flex items-center justify-center shadow-[6px_6px_0_#000] group-hover:rotate-12 transition-transform",
                        cat.id === 'all' ? "bg-white" : cat.color
                      )}>
                        <Icon className={cn("w-8 h-8", cat.id === 'all' ? "text-primary" : "text-white")} />
                      </div>
                      <div className="space-y-3">
                        <h3 className={cn(
                          "text-2xl md:text-3xl font-black uppercase leading-tight",
                          cat.id === 'all' ? "text-white" : "text-foreground"
                        )}>{cat.name}</h3>
                        <p className={cn(
                          "text-sm font-bold leading-relaxed",
                          cat.id === 'all' ? "text-white/80" : "text-muted-foreground"
                        )}>{cat.desc}</p>
                      </div>
                    </div>
                    ...
                    <div className={cn(
                      ...
                      cat.id === 'all' ? "text-white" : "text-primary"
                    )}>
